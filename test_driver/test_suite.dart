...
import 'integration/page_objects/screens/details_test_screen.dart';
import 'integration/page_objects/screens/home_test_screen.dart';

String email = 'don@donandann.com';
...
class TestSuite {
  static void main() {
    group('Todo App Test', () {
      ...
      test(
          'should show a loading screen while the todos are fetched', () async {
        expect(await homeScreen.isLoading(), isTrue);
      });

      test('should sign on with an actual user', () async {
        expect(await homeScreen.isReady(), isTrue);

        final profileScreen = homeScreen.tapProfileButton();
        final signInScreen = profileScreen.tapSignInProfile();
        expect(await signInScreen.isReady(), isTrue);

        await signInScreen.tapEmailSignInButton();
        await signInScreen.enterSignInEmailField(email);
        await signInScreen.enterSignInPasswordField(password);
      ...
      test('should start with a list of Todos', () async {
        expect(await homeScreen.isReady(), isTrue);
        expect(await homeScreen.todoList
            .todoItem('1')
            .isVisible, isTrue);
        expect(await homeScreen.todoList
            .todoItem('2')
